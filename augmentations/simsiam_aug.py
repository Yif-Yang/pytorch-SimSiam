...
class SimSiamTransform():
    def __init__(self, image_size, mean_std=imagenet_mean_std):
        ...
        p_blur = 0.5 if image_size > 32 else -1
        ...
        self.transform_simple = T.Compose([
            T.ToTensor(),
            T.Normalize(*mean_std)
        ])
    def __call__(self, x):
        x1 = self.transform(x)
        x2 = self.transform(x)
        # x0 = self.transform_simple(x)
        return x1, x2
class SimSiamTransform_test(SimSiamTransform):
    def __call__(self, x):
        x1 = self.transform(x)
        x2 = self.transform(x)
        x11 = self.transform(x1)
        x21 = self.transform(x2)
        # x0 = self.transform_simple(x)
